  ...
  const handleAiSuggestions = async () => {
    ...
    try {
      ...
      const historicoPromessas = resumo.categorias.map(async (cat) => {
        const hist = await orcamentoDb.getHistoricoGastos(ent.id, cat.categoria_id, 3);
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Planeje seus gastos e acompanhe sua evolução.</p>
        </div>
            ...
            <div key={cat.categoria_id} className="p-6 hover:bg-gray-50/50 transition-colors">
              ...
              <div className="relative pt-1">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <span className={`text-[10px] font-black inline-block py-1 px-2 uppercase rounded-full ${
                      cat.raw_percentual >= 100 ? 'bg-red-100 text-red-600' : 
                ...
                <div className="overflow-hidden h-2.5 text-xs flex rounded-full bg-gray-100">
                  <div 
                    ...
                    className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center transition-all duration-1000 ${
                      cat.raw_percentual >= 100 ? 'bg-red-500' : 
