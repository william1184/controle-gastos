...
export default function Configuracoes() {
  ...
  const [novoGastoTipo, setNovoGastoTipo] = useState('Variável');
      ...
      try {
        ...
        if (config.categoriasGastos) {
          ...
          if (importedCats.length > 0 && typeof importedCats[0] === 'string') {
            importedCats = importedCats.map(c => ({ nome: c, tipo: 'Variável' }));
          }
  ...
  const addItemGasto = () => {
    if (novoGasto && !categoriasGastos.find(c => c.nome === novoGasto)) {
      setCategoriasGastos([...categoriasGastos, { nome: novoGasto, tipo: novoGastoTipo }]);
      setNovoGasto('');
    }
  };

  const removeItemGasto = (itemNome) => {
    setCategoriasGastos(categoriasGastos.filter(i => i.nome !== itemNome));
  };
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6 max-w-3xl">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <select value={novoGastoTipo} onChange={(e) => setNovoGastoTipo(e.target.value)} className="p-2 border border-gray-300 rounded bg-white">
        ...
        <ul className="flex flex-wrap gap-2">
          {categoriasGastos.map(cat => (
            <li key={cat.nome} className="bg-gray-200 px-3 py-1 rounded-full flex items-center gap-2">
              {cat.nome} <span className="text-xs text-gray-500">({cat.tipo})</span>
              <button onClick={() => removeItemGasto(cat.nome)} className="text-red-500 font-bold hover:text-red-700">&times;</button>
            </li>
          ))}
        </ul>
      </div>
