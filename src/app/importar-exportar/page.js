...
import { addEntrada, getEntradas } from '@/lib/entradasDb';
import { addSaida, getSaidas } from '@/lib/saidasDb';
...
export default function ImportExport() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (loadedPerfis.length === 0) {
        loadedPerfis = [{ id: 0, nome: 'Perfil Padrão', entrada: 1200, dataNascimento: '1992-04-27' }];
      }
  ...
  const handleConfirmMapping = async () => {
    ...
    if (novosSaidas.length > 0 || novasEntradas.length > 0) {
      for (const g of novosSaidas) {
        await addSaida(g);
      }
      for (const r of novasEntradas) {
        await addEntrada(r);
      }
      ...
      alert(`Extrato importado com sucesso!\n\n${novosSaidas.length} saidas adicionados.\n${novasEntradas.length} entradas adicionadas.`);
    } else {
