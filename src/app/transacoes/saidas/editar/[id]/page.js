...
export default function EditarSaida() {
    ...
    const loadData = async () => {
      ...
      if (id) {
        ...
        if (saida) {
          ...
          setCategoria(saida.categoria || (cats.length > 0 ? cats[0].nome : 'Outros'));
          setUsuarioId(saida.usuarioId !== undefined && saida.usuarioId !== null ? saida.usuarioId : loadedUsuarios[0].id);
          setTipoCusto(saida.tipoCusto || 'Variável');
          setTotal(saida.total);
          setContaId(saida.contaId || (loadedContas.length > 0 ? loadedContas[0].id : ''));

          if (saida.recorrenciaId) {
            const recorrencia = await getRecorrenciaById(saida.recorrenciaId);
            if (recorrencia) {
  ...
  const handleSave = async () => {
    ...
    if (isRecorrente) {
      saidaData.recorrencia = { frequencia };
    }

    await updateSaida(Number(id), saidaData);
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-gray-800">Editar Saida</h1>
